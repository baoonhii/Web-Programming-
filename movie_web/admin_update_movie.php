<?php
   ...
   if(isset($_POST['update-product'])){
      ...
      $update_product = $conn->prepare("UPDATE `movies` SET name = ?, category = ?, details = ?, price = ?, trailer = ? WHERE id = ?");
      $update_product->execute([$name, $category, $details, $price, $trailer, $pid]);
...
<section class="update-product">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="number" name="price" min="0" placeholder="Enter ticket price" required class="box" value="<?= $fetch_movies['price']; ?>">
      <input type="text" name="trailer" placeholder="Enter trailer of movie" required class="box" value="<?= $fetch_movies['trailer']; ?>">     
      <select name="category" class="box" required>
         <option selected><?= $fetch_movies['category']; ?></option>
               <option value="tshirt">2D</option>
               <option value="pant">3D</option>
               <option value="shirt">4D</option>
               <option value="polo">5D</option>
      </select>
      ...
      <div class="flex-btn">
         <input type="submit" class="btn" value="Update movie" name="update-product">
