...
<section class="movies">
   <h1 class="title">Movies Showing</h1>
   ...
   <form action="" class="box" method="POST">
      ...
      <input type="hidden" name="trailer" value="<?= $fetch_movies['trailer']; ?>">
      <input type="hidden" name="p_price" value="<?= $fetch_movies['price']; ?>">
      <input type="hidden" name="p_image" value="<?= $fetch_movies['image']; ?>">
      <input type="number" min="1" value="1" name="p_qty" class="qty">
      <div>
         <a href="<?= $fetch_movies['trailer']; ?>" class="option-btn">Trailer</a>
      </div>
      <input type="submit" value="Book Ticket" class="btn" name="add_movie">
   </form>
   <?php
      }
   }else{
      echo '<p class="empty">No Movie!</p>';
   }
   ?>
   </div>
</section>
